...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	// 座席ごとの状況を取得
	rows, err := db.Query("SELECT s.id, s.rank, s.num, r.id, r.user_id, r.reserved_at FROM sheets AS s LEFT OUTER JOIN reservations AS r ON s.id = r.sheet_id AND r.event_id = ? AND NOT r.is_canceled", eventID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var sheet Sheet
		if err := rows.Scan(&sheet.ID, &sheet.Rank, &sheet.Num, &reservationID, &userID, &reservedAt); err != nil {
			return nil, err
		}
		if reservationID.Valid {
			sheet.ReservationID = reservationID.Int64
...
func main() {
	...
	e.GET("/api/users/:id", func(c echo.Context) error {
		...
		// 先に、直近に予約・キャンセルしたイベント一覧を取得
		rows, err := db.Query("SELECT MAX(IFNULL(canceled_at, reserved_at)), e.id FROM events AS e INNER JOIN reservations AS r ON r.user_id = ? AND e.id = r.event_id GROUP BY event_id", userID)
		if err != nil {
			return err
		}
		defer rows.Close()
		
		var events []*EventUpdateInfo
		for rows.Next() {
			var ei EventUpdateInfo
			if err := rows.Scan(&ei.updatedAt, &ei.eventID); err != nil {
				return err
			}
		...
		var recentEvents [5]*Event
		var eventMap map[int64]*Event = make(map[int64]*Event)
		var recentEventCnt int
		if len(events) > 5 {
			recentEventCnt = 5
		} else {
			recentEventCnt = len(events)
		}

		for i := 0; i < recentEventCnt; i++ {
			recentEvents[i], err = getEvent(events[i].eventID, userID)
			if err != nil {
				return err
			}
			eventMap[events[i].eventID] = recentEvents[i]
		}

		// キャンセル日時、または予約日時が新しい順に最大五件の予約情報を取得する
		rows, err = db.Query("SELECT r.id, r.event_id, r.sheet_id, r.user_id, r.reserved_at, r.canceled_at, s.rank, s.num FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id WHERE r.user_id = ?", userID)
		if err != nil {
			return err
		}
		defer rows.Close()
		var reservations []*ReservationUpdateInfo

		for rows.Next() {
			var r ReservationUpdateInfo
			r.reservation = new(Reservation)
			if err := rows.Scan(&r.reservation.ID, &r.reservation.EventID, &r.reservation.SheetID, &r.reservation.UserID, &r.reservation.ReservedAt, &r.reservation.CanceledAt, &r.rank, &r.sheetNum); err != nil {
				return err
			}
			if r.reservation.CanceledAt == nil {
				r.updatedAt = r.reservation.ReservedAt
			} else {
				r.updatedAt = r.reservation.CanceledAt
			}
			reservations = append(reservations, &r)
		}
		sort.Slice(reservations, func(i, j int) bool { return reservations[i].updatedAt.After(*reservations[j].updatedAt) })
		
		var recentReservations [5]*Reservation
		var recentReservationCnt int
		if len(reservations) > 5 {
			recentReservationCnt = 5
		} else {
			recentReservationCnt = len(reservations)
		}
		for i := 0; i < recentReservationCnt; i++ {
			reservation := *reservations[i].reservation
			event := *eventMap[reservation.EventID]
			reservation.Event = &event
			reservation.SheetRank = reservations[i].rank
			reservation.SheetNum = reservations[i].sheetNum
			reservation.Price = event.Sheets[reservations[i].rank].Price
			reservation.ReservedAtUnix = reservations[i].reservation.ReservedAt.Unix()
			if reservations[i].reservation.CanceledAt != nil {
				reservation.CanceledAtUnix = reservations[i].reservation.CanceledAt.Unix()
			}
		...
		for i := 0; i < len(reservations); i++ {
			if reservations[i].reservation.CanceledAtUnix != 0 {
				totalPrice += reservations[i].reservation.Price
			}
		}
		...
		return c.JSON(200, echo.Map{
			...
			"recent_reservations": recentReservations[:recentReservationCnt],
			"total_price":         totalPrice,
			"recent_events":       recentEvents[:recentEventCnt],
		})
	}, loginRequired)
	...
	e.DELETE("/api/events/:id/sheets/:rank/:num/reservation", func(c echo.Context) error {
		...
		var reservationID int64
		found := false
		for _, v := range ec.event.Sheets[rank].Detail {
			if v.Num == num {
				if !v.Reserved {
		...
		if _, err := db.Exec("UPDATE reservations SET canceled_at = ?, is_canceled = TRUE WHERE id = ?", canceledAt.Format("2006-01-02 15:04:05.000000"), reservationID); err != nil {
			return err
		}
		go func(){
			reservationCanceledAt[reservationID] = &canceledAt
